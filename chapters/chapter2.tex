...
\footnotetext[1]{ En este trabajo, utilizamos el mismo símbolo $T$ para denotar el conjunto de transiciones -\textit{transiciones}-
...
un nuevo marking $M'$, esto se nota \firing{M}{t}{M'}.
...
\todo[linecolor=red,backgroundcolor=red!25,bordercolor=red]{esto acá queda feo hay que ponerlo en otor lado o escribir algo más o distinto...}
...
Al relacionar el comportamiento de $\pmlog$, $\Language(N)$ y $\sys$, se definen nuevos conceptos~\cite{BuijsDA14}
...
\begin{definition}
    \todo[]{Si no puedo hacer andar este símbolo,poner $\lvertneqq$ o simplemente $<$}
    \label{def:neg}
