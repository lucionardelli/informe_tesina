...
En la~\autoref{sec:3.simplification} se vio como la técnica presentada es universal y puede ser aplicada sobre
cualquier modelo presentado como una red de Petri. Para probar esto, se toman los modelos obtenidos
mediante \textit{ILP miner}~\cite{WDHS08} para el mismo conjunto de benchmarks y se los simplifica mediante
\pachtool.
...
modelos generados por ILP miner.

Los resultados sobre el tiempo computacional se muestran en la~\autoref{tab:ilp_time}.
La primer columna corresponde al tiempo requerido para obtener la red inicial mediante ILP miner; las demás columnas muestran
los tiempos de simplificación obtenidos por \pachtool. Para todos los casos que no incluyen proyección, \pachtool
es considerablemente más rápido que ILP miner. %Para el caso del benchmark \bench[2]{DbMut}, 
...
para en la~\autoref{sec:4.supervised}.
En cuanto a la generalización, los resultados de la~\autoref{tab:ilp_gen}, muestran que los mejores resultados se obtienen
al aplicar \textsc{Removal} sobre el modelo obtenido luego de la simplificación iterativa mediante SMT. Sin embargo, similares
valores de generalización se obtiene, sin aplicar dicho proceso ($2^{da}$ columna).
